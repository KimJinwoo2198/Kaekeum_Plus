...
class IotControl:
    ...
    def process_command(self, command):
        turn_on_commands = ["불을 켰어", "켰습니다.", "거실 불 켜"]
        turn_off_commands = ["불을 껐어", "껐습니다", "거실 불 꺼"]

        if any(cmd in command for cmd in turn_on_commands):
            return self.turn_on_living_room_light()
        elif any(cmd in command for cmd in turn_off_commands):
